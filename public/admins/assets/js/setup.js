    ...
    CKFinder.popup({
        ...
        onInit: function (finder) {
            finder.on("files:choose", function (evt) {
                var file = evt.data.files.first();
                $(in_value)
                    .val(file.getUrl().replace(urlHome, ""))
                    .trigger("change");
            });
        },
    });
...
function addOverlayLoading(elm) {
    elm = $(elm);
    elm.prepend(
        '<div id="overlayLoading" style="position: absolute;width: 100%;height: 100%;background: #ffffff91;z-index: 10;"></div>'
    );
    elm.find("button[type='submit']").css("opacity", "0.5");
    elm.find("button[type='submit']").prepend(
...
$(document).on("change", ".select2-condition", function (e) {
    ...
    console.log($(target).data("url").split("?")[0] + "?" + param + "=" + val);
    select2LoadData(
        $(target).data("url").split("?")[0] + "?" + param + "=" + val,
        target
    );
});
...
$(document).on("click", ".open-modal-success", function () {
    var form = $("#modalFormSuccess"),
        action = $(this).data("route");
    form.attr("action", action);
});

$(document).on("click", ".open-modal-warning", function () {
    var form = $("#modalFormWarning"),
        action = $(this).data("route");
    form.attr("action", action);
});
